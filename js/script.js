...
for (let i = 0; i < offer__opinions_bottom.length; i++){
    offer__opinions_bottom[i].addEventListener('click', (el)=>{
        ...
        offer__opinions_bottom[i].previousElementSibling.lastElementChild.addEventListener("click", (el)=>{
...
for (let i = 0; i < col__body_title.length; i++) {
    col__body_title[i].addEventListener('click',(e)=>{
        localStorage.setItem(e.target.getAttribute('model'), 'false');
        conpare_visible();
...
for (let i = 0; i < btn_comparison.length; i++) {
    btn_comparison[i].addEventListener('click',(e)=>{
        if (localStorage.getItem(e.target.getAttribute('model')) && localStorage.getItem(e.target.getAttribute('model')) === 'false'){
            localStorage.setItem(e.target.getAttribute('model'), 'true');
            conpare_visible();
